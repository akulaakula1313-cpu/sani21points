...
// === Удаляем старых "без ника" (без имени) ===
async function cleanupEmptyPlayers(){
  if(!useMongo||!playersCol)return;
  let removed=0;
  const idsToDelete=[];
  ...
  if(idsToDelete.length){
    try{await playersCol.deleteMany({_id:{$in:idsToDelete}});}catch(e){console.error('cleanup error:',e.message);}
  }
  if(removed)console.log(`Mongo: удалено ${removed} пустых сессий`);
}
...
// === НЕ СОЗДАЁМ игрока автоматически ===
function getPlayer(req,res,create=false){
  let sid=(req.headers.cookie||'').match(/(?:^|; )sid=([^;]+)/)?.[1];

  // игрок есть — возвращаем
  if(sid&&players[sid]){
    const q=players[sid];
  ...
  // cookie есть, но игрока нет
  if(sid){
    if(!create)return [sid,null];
  ...
  // cookie нет — генерируем новый sid
  const newSid=crypto.randomBytes(18).toString('hex');
  res.setHeader('Set-Cookie',`sid=${newSid}; HttpOnly; SameSite=Lax; Path=/; Max-Age=31536000`);

  if(!create)return [newSid,null];

  players[newSid]={balance:10000,round:0,firstGiftDay:utcDate(),lastGiftDate:null,stats:{win:0,loss:0,push:0},name:'',blocked:false,role:null,unseenGift:0};
  persist(newSid);
  return [newSid,players[newSid]];
}
...
const server=http.createServer(async(req,res)=>{
  ...
  // === health-check БЕЗ создания игрока ===
  if(u==='/api/health'){
    res.writeHead(200,{'Content-Type':'application/json','Cache-Control':'no-store'});
    return res.end('{"ok":true}');
  }

  // === favicon БЕЗ создания игрока ===
  if(u==='/favicon.ico'||u==='/favicon.svg'){
    res.writeHead(204);return res.end();
  }

  // === получаем игрока, но НЕ создаём его автоматически ===
  const[sid,p]=getPlayer(req,res,false);

  // блокировка
  if(p&&p.blocked&&u.startsWith('/api/')&&u!=='/api/me'&&u!=='/api/me/name'&&u!=='/api/me/ack-gift'&&u!=='/api/bot/cancel'&&u!=='/api/admin/login'&&!u.startsWith('/api/admin/'))return send(res,403,{error:'Вы заблокированы администратором.'});

  if(u.startsWith('/api/')){
    try{
      // === /api/me — если игрока нет, отдаём дефолт ===
      if(req.method==='GET'&&u==='/api/me'){
        if(!p)return send(res,200,{balance:10000,round:0,stats:{win:0,loss:0,push:0},name:'',blocked:false,role:false,needName:true,gift:null,unseenGift:0,newUser:true});
        return send(res,200,{balance:p.balance,round:p.round,stats:p.stats,name:p.name,blocked:p.blocked,role:p.role==='admin',needName:!p.name,gift:giftInfo(p),unseenGift:p.unseenGift||0});
      }

      // === /api/me/ack-gift ===
      if(req.method==='POST'&&u==='/api/me/ack-gift'){
        if(!p)return send(res,200,{ok:true});
        p.unseenGift=0;persist(sid);
        return send(res,200,{ok:true});
      }

      // === /api/me/name — ЗДЕСЬ создаём игрока ===
      ...
      if(req.method==='POST'&&u==='/api/admin/login'){
        ...
        if(q.pass===ADMIN_PASS){
          if(!p){
            players[sid]={balance:10000,round:0,firstGiftDay:utcDate(),lastGiftDate:null,stats:{win:0,loss:0,push:0},name:'',blocked:false,role:'admin',unseenGift:0};
            p=players[sid];
          } else {
            p.role='admin';
          }
      ...
      if(req.method==='POST'&&u==='/api/admin/logout'){
        if(p){p.role=null;persist(sid);}
        return send(res,200,{ok:true});
      }

      // === админка ===
      if(u.startsWith('/api/admin/')){
        if(!p||p.role!=='admin')return send(res,403,{error:'Доступ запрещён.'});

        // Скрываем "без ника" — показываем только реальных игроков
      ...
      // === остальные API — если игрока нет, отказ ===
      if(!p)return send(res,403,{error:'Сначала задайте никнейм.'});
...
async function main(){
  ...
  // очистка "без ника"
  try{await cleanupEmptyPlayers();}catch(e){console.error('cleanup error:',e.message);}
